...
namespace AssaltCubeGameHack
{
    internal class PlayerData
    {
        ...
        int weapon1_offset = 0x164; // 1번 총
        int weapon2_offset = 0x150; // 2번 총
        int weapon4_offset = 0x14C; // 4번 칼
        ...
        {
            mem.WriteFloat(base_addr + y_pos_offset, 10.0f); // 기존 높이에서 10만큼 점프
        }

        internal void reset(ProcessMemoryReader mem)    // 슈퍼점프 해제하고 원 상태로 돌아오도록 하기
        {
            mem.WriteFloat(base_addr + y_pos_offset, 2.0f);
        }

        internal void hackheal(ProcessMemoryReader mem) // 구급상자의 위치로 이동
        {
        ...
        {
            mem.WriteInt(base_addr + weapon1_offset, 30); // 1번 총 공격속도 상승
            mem.WriteInt(base_addr + weapon2_offset, 30); // 2번 총 공격속도 상승
            mem.WriteInt(base_addr + weapon4_offset, 30); // 4번 칼 공격속도 상승
        }
